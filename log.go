...
// LoggerAdapter is an interface, that you need to implement to support Shard
// logging.
//
...
// StdLoggerAdapter is a logger implementation, which sends all logs to
// provided standard output.
...
// NewStdLoggerWithOut creates StdLoggerAdapter which sends all logs to
// provided io.Writer.
func NewStdLoggerWithOut(out io.Writer, debug bool, trace bool) LoggerAdapter {
	l := log.New(out, "[shard] ", log.LstdFlags|log.Lmicroseconds|log.Lshortfile)
...
func (c *CaptureLoggerAdapter) PrintCaptured(t Logfer) {
	...
	for level, messages := range c.captured {
		for _, msg := range messages {
			t.Logf(
				"%s %d %s %v",
				msg.Time.Format("15:04:05.999999999"),
				level,
				msg.Msg,
				msg.Fields,
			)

...
// default. Generate it by subtracting 4 levels from [slog.Debug] following
// the example of [slog.LevelWarn] and [slog.LevelError] which are set to 4
// and 8.
...
func (s *SlogLoggerAdapter) log(level slog.Level, msg string, args ...any) {
	mappedLevel, ok := s.shardLevelToSlog[level]
	if ok {
		level = mappedLevel
	}
...
// NewSlogLoggerWithLevelMapping creates an adapter to the standard library's
// structured logging package. A `nil` logger is substituted for the result of
// [slog.Default].
//
// The `shardLevelToSlog` parameter is a map that maps Watermill's log levels
// to the levels of the structured logger. It's helpful, when want to for
// example log Watermill's info logs as debug in slog.
func NewSlogLoggerWithLevelMapping(
	...
	return &SlogLoggerAdapter{
		slog:             logger,
		shardLevelToSlog: shardLevelToSlog,
	}
}
